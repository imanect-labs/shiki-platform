...
test("新規ドキュメント→Collabora エディタが起動し文書が編集できる", async ({ page }) => {
  ...
  // 文書ロード失敗（WOPI 不達等）時はスケルトンのまま Viewing に落ちるため、
  // 「Editing」モード表示まで確認して初めてロード成功とみなす（UserCanWrite の検証も兼ねる）。
  await expect(frame.getByText("Editing", { exact: true }).first()).toBeVisible({
    timeout: 60_000,
  });
});
...
test("Office 未対応拡張子は開くとダウンロードにフォールバックする", async ({ page }) => {
  ...
  // アップロード直後は一覧の再描画で行が差し替わるため、検索で絞り込んでからクリックする。
  await page.getByPlaceholder("ドライブを検索").fill(name);
  await expect(page.getByText(name, { exact: true })).toBeVisible({ timeout: 20_000 });
