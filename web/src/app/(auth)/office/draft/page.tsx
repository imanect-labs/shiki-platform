...
/// 下書き Word 文書画面（#332）。/office/draft?thread=&name= で未保存の下書きを詰める。
///
/// - 下書きは**クライアント内のみ**（サーバ未作成・[`documentDraftStore`] が真実源）。チャットで
///   「Word で〜を作って」→ save_document が document_draft を返し、本画面へ遷移して本文を用意する。
/// - 同じ会話の複数下書きは**上部タブ**で切替（別名 = 別下書き・N 本並存）。
/// - 右上「ドライブに保存」で POST /documents（blank.docx + append_markdown で .docx 化）→
///   実体化して /office/{id}（Collabora）へ。
/// - アシスタント（同じ会話）を同席させ、保存前に AI と詰められる（流し込みで再シード）。
///   本文の編集は md（DraftNoteEditor）で行い、.docx への変換は確定保存時にサーバ側で行う。
...
import { documentDraftStore, parseDocumentDraft } from "@/lib/documents/draft";
...
export default function DraftDocumentPage() {
  return (
    <React.Suspense fallback={<div className="h-full" />}>
      <DraftDocumentPageInner />
    </React.Suspense>
  );
}

function DraftDocumentPageInner() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const threadId = searchParams.get("thread") ?? "";
  const nameParam = searchParams.get("name") ?? "";

  const drafts = documentDraftStore.useDrafts(threadId);
  ...
  const openDraft = React.useCallback(
    (name: string) => {
      ...
      setActiveName(name);
      setSeed({
        markdown: documentDraftStore.get(threadId, name)?.content ?? "",
  ...
  // 会話履歴の document_draft ブロックから復元する（リロード/別端末・note と同型）。ローカルに
  // 一部だけ残っている場合も履歴を常に取得し、**未登録の (threadId, name) のみ**追加する
  // （ローカルのユーザー編集は上書きしない）。
  React.useEffect(() => {
    if (!threadId) {
      setRecovered(true);
  ...
  React.useEffect(() => {
    if (!recovered) return;
    const list = documentDraftStore.list(threadId);
  ...
  const onChangeMarkdown = React.useCallback(
    (markdown: string) => {
      if (activeName) documentDraftStore.upsert(threadId, activeName, markdown, "user");
    },
    [threadId, activeName],
  );

  // アシスタントが同じ会話で流し込んだとき（document_draft）は、その下書きをアクティブにして再シード。
  const onDocumentDraftOpened = React.useCallback((name: string) => openDraft(name), [openDraft]);
  ...
  const doSave = React.useCallback(
    (target: SaveTarget) => {
      ...
      const markdown = editorRef.current
        ...
        : (documentDraftStore.get(threadId, activeName)?.content ?? "");
      createDocument({ parentId: target.parentId, name: target.name, markdown })
        .then((node) => {
          documentDraftStore.remove(threadId, activeName);
          toast({ description: `「${node.name.replace(/\.docx$/i, "")}」を保存しました。` });
          // 実体化した Word 文書を Collabora エディタで開く。
          router.replace(`/office/${node.id}`);
        })
        .catch((e: unknown) => {
          toast({
            description: e instanceof Error ? e.message : "保存に失敗しました。",
          });
          setSaving(false);
        });
    },
  ...
  if (!threadId) {
    return (
      <EmptyState
        title="下書きが見つかりません"
        description="チャットから「Word で資料を作って」と頼むと、ここに下書きが用意されます。"
      />
    );
  }
        ...
        {chatOpen && (
          ...
          >
            ...
            <div className="min-h-0 flex-1">
              {/* 下書き画面では遷移せずアクティブ下書きを切替（onDocumentDraftOpened）。 */}
              <Conversation
                threadId={threadId}
                variant="panel"
                onDocumentDraftOpened={onDocumentDraftOpened}
              />
            </div>
          </FadeSlide>
        )}
