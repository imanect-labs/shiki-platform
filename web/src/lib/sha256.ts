/// SHA-256（hex）。`crypto.subtle` が使える時はそれを使い、使えない時は純 JS 実装に落ちる。
///
/// `crypto.subtle` は **セキュアコンテキスト**（HTTPS / localhost）でしか露出しない。
/// dev で HTTP かつ非 localhost ホスト（例: shuya-dev:10067）から開くと undefined になり、
/// アップロードのハッシュ計算が失敗する。フォールバックでその環境でも動かす。

/// バイト列の SHA-256 を hex 文字列で返す。
...
// --- 純 JS SHA-256（FIPS 180-4） ----------------------------------------

// 最初の 64 個の素数の立方根の小数部（定数 K）。
...
function sha256Bytes(msg: Uint8Array): Uint8Array {
  ...
  for (let i = 0; i < 8; i++) outDv.setUint32(i * 4, h[i] >>> 0, false);
  return out;
}
