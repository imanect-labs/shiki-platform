...
| 経路 | loadPyodide p50 | 備考 |
|---|---|---|
| フルブート（従来） | **~1,900ms** | この開発機は bench.md の LXC ホストより高速（同条件比較のみ有効） |
| スナップショット復元 | **~410ms（うちチャンク読込 ~195ms）** | **~4.6×** 短縮 |
| スナップショット作成 | ~3,900ms | アセット指紋ごとに 1 回だけ（prewarm 内） |

スナップショットサイズ ~21MB（≒ RSS 面でも取り回し可能）。bench.md の exec ~6.5s 環境なら
支配項がこの比率で縮む見込み。**正式な 3 ティア比較は dev ホストで `SANDBOX_BENCH=1` の再計測で更新すること**。

検証: `crates/execution/tests/python_snapshot.rs`（作成→復元→再利用、無効化フォールバック、
**復元後のランタイム状態パリティ**＝cwd/env/argv/version/例外挙動がフルブートと一致）。

## ロードマップ（次の削り代）

1. **チャンク読込の型付き搬送**（~195ms→数十ms）: sync-RPC の base64 経由をやめ、
   `wasm_module_bytes` と同様の typed バイナリチャネルでスナップショットを渡す。
2. **micropip / preload パッケージ込みスナップショット**: 現在はベースライン（ブート直後）のみ。
   Cloudflare の package snapshot に相当する多段指紋（preload 構成をキーに含める）で
   `packageLoadMs` も除去できる。JS FFI 状態の直列化制約に注意。
3. **pyodide.asm.wasm の V8 コンパイルキャッシュ**: `WebAssembly.Module` の serialize/deserialize
   （V8 code cache）で restore 経路の残コストを削る。
4. **web_fetch の native 経路**: urllib(Python) を経ない fetch 実装（design §4.6 の既知課題）。
   スナップショットで Python 経路自体が ~1s 級まで縮むため優先度は下がるが、egress モデルは不変のまま
   さらに桁を落とせる。
5. **温機プール**: orchestrator 側で復元済み VM をプールすれば体感 create+exec を数十 ms 級へ。
   PIT-22（時間衝突）の再評価が前提。
