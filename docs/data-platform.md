...
④で必ず IR に落ちるので、**SQL 面を足しても攻撃面は増えない**。IR は既に閉じた文法であり、
...
採用方式と同じ出典から取れる**。実装面でも `crates/tabular/src/sql_guard.rs::validate_read_only`
（パースして閉じた部分集合に照合し拒否する）が同型の前例。
...
**JOIN を宣言済みリンク列に限る理由**が重要である。任意の結合を許すと参照先の行ポリシーを
どう伝播させるかが破綻する。リンク列経由なら `data_link` の 2 行方式でパーティション枝刈りが効き、
参照先の行述語も §7.2 の lookup と同じ経路で掛かる。**authz の観点から JOIN の自由度を制限する。**
...
| **JOIN 除去** | 結合先の列を `SELECT` にも `WHERE` にも使っていなければ、存在チェックへ落とす／消す |
...
`data_table.schema` は全クエリで引かれる。**`schema_version` が既にある**ので、
これをキーにしたプロセス内キャッシュが安全に置ける（スキーマは権限ではないのでキャッシュ禁止の対象外）。
地味だが全クエリに効く。
...
- **COPY ベースのバルクパス**（索引テーブルへも COPY）
- **チャンク分割トランザクション**——ロック保持時間を切る。1 Tx で 10 万行を抱えない
- **索引の遅延構築**——本体を入れてから索引をまとめて構築する経路（§10 の `building` 状態を流用）
