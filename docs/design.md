...
- **web ツール**: 新トレイト `SearchProvider`（SaaS=Brave Search API / オンプレ=SearXNG / エアギャップ=機能無効）。
  ページ取得はサンドボックス経由（egress allowlist に検索プロバイダを登録）。
...
- **購読**: `GET /runs/:id/events`（SSE/EventSource）。**購読開始→ `Last-Event-ID`(=seq) から DB リプレイ→
  seq で重複破棄**の順（リプレイ→ライブの隙間ゼロ）。どのインスタンスでも Redis 経由で受信。
...
- **フィードバック（テナント境界を跨ぐ唯一のデータフロー）**: 二段構え。
  ①既定=メタデータのみ（評価＋カテゴリ＋コメント＋run_id/trace_id・**会話本文なし**）を共有コントロールプレーンの
  feedback ストアへ。ベンダー側は Langfuse/監査と trace_id 突合（本文は見えない）。
  ②本文添付=報告ダイアログでのユーザー明示同意時のみ。さらに**組織管理者ポリシーで本文添付を禁止可能**（二重ゲート）。
  添付の事実は監査記録。👎カテゴリ「やり方がわからなかった」をヘルプの穴検出器に使う。
