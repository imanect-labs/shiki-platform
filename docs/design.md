...
- 機能は必要分のみ（フォールバック/リトライ/トークン会計/Langfuse計装/権限注入）。
  **ルールベースのコストルータは §4.5.1 で導入**。学習型ルーティング・セマンティックキャッシュ・仮想キーは後追い。
- **モデルカタログ**: テナント管理者が「許可モデルリスト＋既定モデル＋モデル別単価（課金単価表と同居）＋
  国外処理バッジ（データレジデンシ明示）」を管理。ユーザーのモデル選択 UI はカタログの範囲内。
  skill のモデル既定（FR-7）もここに整合。カタログの各エントリは
  **能力メタデータ（tier・context 窓・tools/vision 対応・residency・provider）** を併せ持ち、§4.5.1 の候補集合の素になる。
- **複数プロバイダを同時に持てる**（`providers[]` ＋ `ModelEntry.provider`）。
  「ローカル vLLM を軽いタスク／外部 API を難しいタスク」という振り分けはコスト削減の最大手段であり、
  provider 単数のままでは表現できない。単数設定は `name="default"` の 1 要素として読む（後方互換）。
  外部 provider を構成しない環境では該当モデルが候補から消える（**NFR-2 エアギャップ無傷**）。
...
### 4.5.1 モデルルータ（ルールベース・コスト最適化）

> 詳細設計の正本は [docs/llm/model-router.md](./llm/model-router.md)。実装は
> [roadmap トラック LR](./roadmap/parallel-tracks.md)。着手前に [PIT-45・PIT-46](./design-caveats.md) を確認すること。

**目的はコストカットのみ**（品質最大化ではない）。学習型ルータに要る選好データが無い段階の現実解として、
**決定的なルールだけ**で「十分な能力を満たす候補のうち最安」を選ぶ層を `LlmGateway::stream()` の内側に置く。
呼出点（agent-core / chat / workflow llm ノード / app-gateway `/ai`）は全て gateway を通るため、
単一チョークポイント（§1）を壊さずに 1 箇所で効く。

- **ルータは候補集合の中から選ぶだけ**。候補集合＝カタログ ∩ 能力要件（tools/vision/context 窓）∩ 呼出元の許可
  ∩ レジデンシ制約。**認可境界もレジデンシ境界も広げない**（降格は常に許可集合の内側で起きる）。
- **明示選択（ユーザー / skill のモデル既定 / ノード指定 / アプリ指定）は上書きしない。**
- **ルータ自身は LLM を呼ばない**（推論を呼んだ時点で削減が消える）。判断材料は呼出側が宣言する
  `TaskKind` ＋ 入力トークン概算 ＋ ツール提示の有無 ＋ `effort` ＋ 文字列走査のみ。
- **同一 run 内で tier は単調非減少（粘着）**。ステップ毎に最安を選び直すとプレフィックスキャッシュが壊れ、
  再計算が削減額を上回りうる。安価で始めて失敗の兆候（JSON 破損・空応答・MaxTokens 未完・ループ）が出たら
  1 段昇格して同ステップを再生成する（上限付き・判断は agent-core、ルータは純関数）。
- **会計は実効モデルで刻む**: `stream()` が実効モデルを戻り値で返し、`llm_usage` に
  `requested_model` / `baseline_cost_usd_micros` / `route_rule` / `route_mode` を持つ。
  **削減額＝baseline − actual** を SQL で出せる（金額クリティカル・[PIT-45](./design-caveats.md)）。
- **段階導入**: `router.mode = off | shadow | on`、**既定 `shadow`**（実行は従来どおり・決定と想定額だけ記録）。
  実データで削減率と妥当性を確認してから `on` にする。テナント単位で上書き可（Task 12.1）。
- **将来**: `Router` はトレイト。昇格の有無が正例/負例のラベルになるため、**運用そのものが学習型ルータの
  教師データを生む**。差し替えは同一トレイト裏＋shadow の A/B で行う（§3.1 の配置差トレイトとは層が別）。
