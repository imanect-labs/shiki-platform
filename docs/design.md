...
**web_fetch のような「Python を使わない・egress 限定の短命実行」で引き続き使う**。
...
flowchart TB
  ...
  TIER -->|web_fetch 等・Python を使わない短命実行| WASM["wasm: secure-exec フォーク<br/>vendor/secure-exec・per-sandbox 非特権 sidecar 子プロセス<br/>V8＋Pyodide・仮想FS/PTY/仮想net内蔵"]
...
| wasm | web_fetch（egress 限定・短命）・Python を使わない軽量実行 | プロセス分離＋wasm 二層（ブラウザ級・VM級ではない） | ✅ |
...
- **ティア選択の導線（admin ポリシー）**: コード実行系（code_interpreter / agent shell / workflow の agent_invoke）の
  隔離ティアは server 設定 `chat.sandbox_backend`（`gvisor`（既定）/ `wasm` / `firecracker`）で選ぶ。ユーザー/ノード設定には
  ...
  gVisor にした根拠（[bench](./sandbox/bench.md)）。**web_fetch は egress 限定の短命 sandbox のため常に wasm**（この設定の対象外・
