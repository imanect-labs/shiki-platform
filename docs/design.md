...
- **ティア選択の導線（admin ポリシー）**: コード実行系（code_interpreter / agent shell / workflow の agent_invoke）の
  ...
  **2026-07-26**: この初期化コストは Pyodide ヒープスナップショット（作成 1 回・以後は復元でブート回避・
  typed 搬送・micropip/stdlib pre-import 同梱・プロセス内 wasm コンパイル共有・
  [sandbox/wasm-exec-optimization.md](./sandbox/wasm-exec-optimization.md)）で
  **Python 起動全体 ~2.34s → 初回 ~0.5s / 同一プロセス2回目以降 ~0.40s** に短縮済み。既定有効・
