...
- なぜ ReBAC か:「ロールの階層」「フォルダの継承」「個別共有」を自然に表現でき、**RBAC ロールの組合せ爆発**を避けられるから。
  （対比: **RBAC** = ロールに権限を割り当てる方式。ロール階層＋個別共有があると RBAC ロールが爆発するのでコアには採用しない。）
...
| `identity` | ユーザー/ロール情報の参照 | `identity.read` |
