...
> 対象: 新しくアサインされたエンジニア。ミニアプリ／業務アプリ基盤（打倒kintone）の全体像と、その前提になる
...
- **ミニアプリ = 社内の業務アプリ**（申請、台帳、案件管理…）。狙いは **kintone の置き換え**。
- 2種類ある:
...
その上で「**現場が自分たちの業務アプリを増やしていける**」状態を作ると、価値が一気に伸びます。これは **kintone** がやっていることそのものです。
...
- フィールド型に **user / dept / file / record 参照**を持てる（kintone のユーザー選択・関連レコードに相当）。
...
この WHERE 強制注入・クエリコンパイラは**正しさがクリティカル**（authzバイパス・集計リークが起きたら事故）なので、**fable 5 に委譲**する領域です。
...
| 場所 | 役割 | 備考 |
|------|------|------|
| `crates/app-gateway/` | 公開APIゲートウェイ・OAuth2/スコープ・能力面 | 二重ゲート・token-exchange（**fable 5 委譲**） |
| `crates/app-platform/` | ミニアプリ artifact・マニフェスト・レジストリ・FSM | 人が握る |
| `crates/data/` | 構造化データ・record/schema・**行authz述語エンジン** | クエリコンパイラ（**fable 5 委譲**） |
...
> **fable 5 委譲とは**: 「トレイトの裏側（深く難しく、正しさがクリティカルな所）」を fable 5 に任せ、
> 「トレイトの表側（統合・体験・ポリシ決定）」は人が握る、という分担。
> ミニアプリ基盤では **app-gateway の認可** と **data の行述語エンジン** が委譲対象（どちらも事故ると重大なため）。
...
| fable 5 委譲 | 難所（深い・正しさクリティカル）を fable 5、表側を人が握る分担 |
