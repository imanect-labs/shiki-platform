...
- **仕様**: **ノートページが分割ビューをホスト**し、既存チャットスレッド UI（conversation コンポーネント）を
  ...
  - **follow-up #282**: 「会話→ドキュメント」を整えた。ノート:会話を **1:N**（`thread.origin_note_id`・会話切替/
    リセット・履歴に「ノート由来」）に、`save_note` を**下書き確定型**（下書きノート画面→「ドライブに保存」）に拡張。
    genui 本文挿入は `document.embed`。現行の正は design.md §4.8.1。
- **受け入れ条件**:
...
- **受け入れ条件**:
  ...
  - [ ] script を含む生 HTML が**どの流入経路でも**実行されない（エディタ貼り付け・note_ref 保存・
    ファイル側 md 直接書込→インポート、のすべてで表示のみ）
...
- **仕様**: `csv.query`（RO SQL・ページング結果）／`csv.patch`（パッチ操作→新バージョン）／`csv.write`
  ...
  **要求 relation は操作別**: query=viewer / patch=editor / write=保存先フォルダへの作成権限。
  ワークフローステップは at-least-once 再試行（PIT-31）に備え、**エンジンの冪等キーを tabular 側で消費**して
  書込（patch/write）を重複排除する。ノード型/スコープは workflow IR の閉カタログ・codegen 認可語彙
  （単一定義・手書き禁止）に登録する。
- **受け入れ条件**:
  - [ ] チャットで「この CSV を分析して」が csv.query 経由で動き、引用元が監査に残る
  - [ ] ワークフローから行追記→新バージョン→書込イベントが流れ、**ステップ再試行で行が重複しない**
  - [ ] 実行主体が読めないファイルへの呼び出し・**viewer のみの実行主体による csv.patch／作成権限のない csv.write** が拒否される（negative テスト）
