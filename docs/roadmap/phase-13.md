...
>
> 🔒 **human 承認が要るタスク**:
> - **13.8 の「匿名公開の有効化」** — design §4.13 は匿名公開の解禁自体を PIT-46 と同じ包絡に置いている。
>   **リスナの実装は既定 OFF で進めてよいが、匿名到達を有効化する構成変更は承認事項**。
>   13.8 が 13.12 より先に来るため、これを分けないと Stage 2 と DoD を進めるだけで未承認の匿名閲覧が解禁されてしまう。
>   なお現段階の縮退は「**匿名到達を既定 OFF にしたまま限定共有（Session＋FGA）だけで配る**」ことであり、
>   「閲覧者が同じテナントに属することを匿名リスナが検査する」という意味ではない（匿名リスナは閲覧者テナントを
>   持たない。境界は `Host → site → publication` で解決した**所有テナント基準**で publish 時に検査する・design §4.13）。
...
- **仕様**: `site`（フォルダ node を指すメタ: slug・ドメイン・テーマ・公開設定）と `publication`
  ...
  **publication はそのページが参照するフォーム定義の版もピン留めする** — `form` は可変メタなので、
  不変 HTML が「現在の定義」を引くと再 publish なしに過去ページの宛先・スキーマ・既定値が変わる
  （アセットの参照断ちと同じ穴が定義で開く）。定義変更は再 publish を要求する。
...
- **受け入れ条件**:
  ...
  - [ ] unpublish で配信が止まる
  - [ ] publication がフォーム定義版をピン留めし、定義を編集しても再 publish まで反映されない
...
- **仕様**: `PrincipalKind` に匿名種別を追加し（`PrincipalKind::Workflow` を足した migration 0022 と同型）、
  ...
  **レート制限は workflow-engine の Redis トークンバケット**（Lua でアトミック・キーは自由形式）を基盤に、
  **sites 用のキー契約を新設する** — 既存の呼び出し側は `tenant_id:capability` を使い名前空間も `wf:ratelimit:` で、
  IP や `form_id` の契約は存在しない。決めるのは ①バケットキー（`tenant_id` ＋ 対象種別（IP / `form_id`）＋
  正規化値）②IP 単位・`form_id` 単位・テナント全体の 3 段の上限と適用順序 ③実クライアント IP の入力契約
  （13.6 の trusted-proxy 境界から得た値のみ）④sites 用の名前空間。既存 API で表現できなければ
  キー生成と適用を sites 側の薄い API として足す。
  **受付は送信が属する publication からピン留めされたフォーム定義版を解決して検証**し、
  送信トークンと冪等キーも同じ版に束縛する（版が消えている場合は fail-closed）。
  **冪等性は 2 段構え**（PIT-58）: JS がある経路は受付が発行する短命の署名済み送信トークン、
  no-JS 経路は `(publication_id, form_id, フォーム定義版, 正規化ペイロードのハッシュ, クライアント IP ハッシュ)` を鍵に分単位の窓で重複排除する
...
- **受け入れ条件**:
  ...
  - [ ] IP 単位・`form_id` 単位・テナント全体の 3 段が定義した順序で適用される
  - [ ] 信頼済み hop 以外から来た `X-Forwarded-For` でレート制限を回避できない
  - [ ] フォーム定義を変更しても、再 publish するまで既存 publication の宛先・スキーマ・既定値が変わらない
  - [ ] 同一フォームを複数サイト／複数 publication で使ったとき、別 publication からの同一内容・同一 IP の
        no-JS 送信が互いに重複排除されない（鍵に `publication_id` とフォーム定義版を含む）
  - [ ] 送信ごとに監査が 1 件残る
...
- **仕様**: バリアント割当は配信リスナ側で行うが **cookie は焼かない** — 日次ソルト付きハッシュ（IP＋UA）で
  決定的に算出する。**A/B 有効ページは既定で共有キャッシュに置けない**: 安定 URL のキャッシュキーはバリアント間で
  同一で、publication 単位の ETag ではバリアント差を表せないため、ソルトを切り替えても 304 で古い方が返る。
  したがって**既定は `private, no-store`**（CDN キャッシュを捨てるのが対価）とし、
  **バリアントとソルト世代をキャッシュキーと ETag の両方に含める構成が取れる場合に限り共有キャッシュを許す**
  （どちらか一方だけでは足りない）。
  **プライバシー要件を先に決める**: cookie を焼かないことが避けるのは ePrivacy の「端末への保存/読み出し」への
  同意であって、IP と UA の処理自体は仮名化データの取り扱いとして残る。法的根拠・利用目的・保持期間・
  ソルト（HMAC 鍵）の保護方法・オプトアウト手段を定め、**法務確認が済むまで A/B を既定有効化しない**。
- **受け入れ条件**:
  - [ ] 同一 IP/UA が同日中は同じバリアントに割り当たる
  - [ ] cookie が発行されない
  - [ ] バリアント間でキャッシュが混ざらない（共有キャッシュを挟んだ結合テスト）
  - [ ] 日次ソルト切り替え後に旧バリアントが配信されない
  - [ ] ソルト（HMAC 鍵）が設定として保護され、ログや `site_event` に出ない
  - [ ] 法務確認が未了の間は A/B が有効化できない（既定 OFF）

### Task 13.18: 計測
- **area**: api/frontend / **path**: `crates/site`, `crates/jobq`, `migrations/`, `web/`
- **仕様**: 専用テーブル `site_event`（tenant_id 先頭の複合 PK・
  **`unique(tenant_id, site_id, idempotency_key)`**。`migrations/0013_llm_usage.sql` が手本だが、
  一意制約に `site_id` を含める — `tenant_id` だけで括ると同一テナントの別サイトが同じ鍵を使ったイベントまで
  重複排除され、片方の計測が黙って落ちる）を新設する。**outbox は使わない**（`node_id not null` かつ
  ...
  **ビーコンはフォームと同じ匿名書き込み面**なので 13.12 の境界に載せる（本タスクが 13.12 に依存する理由）:
  ...
  `unique(tenant_id, site_id, idempotency_key)` は同じ鍵の再送しか止めないため、
  **鍵を作り変え続ける相手には `site_event` と jobq を埋められる** — 一意制約を防御と数えない。
  **集計の冪等性は別に用意する**: `jobq` は at-least-once（PIT-31）なので、集計を更新したあと `ack` 前に
  ワーカーが落ちると同じジョブが再配送される。**一意制約が守るのは受信行の重複だけで、集計値の二重加算は防がない**。
  集計は `site_event` を真実として導出する（増分カウンタを持たない）か、増分を持つなら
  **集計更新と「そのイベントを集計済みにする印」を同一トランザクションで行う**
  （RAG が `(node_id, version, op)` を冪等キーに使っているのと同型）。
  **逆向き（未投入）も塞ぐ**: `site_event` を保存したあと enqueue 前に落ちると、行は残るのにジョブが生まれず、
  at-least-once はこれを回復しない（再配送はジョブが存在する場合の話）。`jobq::enqueue_on` を
  **`site_event` の挿入と同一トランザクションで呼ぶ**（トランザクション内投入が `crates/jobq` の本質価値）。
  導出型の集計を採るなら未投入は問題にならないが、増分型を採る場合は
  **未集計行を拾い直す backfill（`aggregated_at IS NULL` の走査）を併せて仕様化する**。
- **受け入れ条件**:
  - [ ] 既定で計測が無効
  - [ ] 生の IP/UA がテーブルに保存されない
  - [ ] 同一冪等キーの二重計上が起きない
  - [ ] 鍵を変え続ける大量送信がレート制限とクォータで止まり、他サイトの計測に波及しない
  - [ ] 同一テナントの別サイトが同じ冪等キーを使っても、双方のイベントが残る（一意制約に `site_id` を含む）
  - [ ] **集計更新後・`ack` 前にワーカーを落として再配送させても、集計値が二重加算されない**
  - [ ] **`site_event` 保存後・enqueue 前に落ちてもイベントが集計から取り残されない**（同一トランザクション投入、
        増分型なら backfill）。回復後も集計への反映は 1 回だけ
