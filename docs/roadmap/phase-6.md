...
> RAG範囲を絞る prompt template、そして両者＋許可ツールを束ねた **ミニアプリ**を、すべて
> 「アーティファクト＋バージョン管理＋ReBAC共有＋監査」の共通枠に乗せる。
> 完了の定義(DoD): 利用者が prompt template（システムプロンプト＋知識スコープ＋許可ツール＋モデル既定）を
> 作って部署に共有でき、チャット応答が generative_ui ブロックとしてフォーム/テーブル/チャート等を描画し、
> そのUIからの操作は**宣言済み・認可済みバックエンドアクション経由のみ**で実行され（アンビエント権限なし）、
> template＋UIスペック＋許可ツールをまとめた**ミニアプリ**が部署で共有・実行できる。
...
| 6.10 | ミニアプリ・アーティファクト（template＋UIスペック＋許可ツール） | gui | – | 6.5, 6.9 |
...
- **仕様**:
  ...
  - OpenFGA に `artifact` 型と relations（viewer/editor/owner）。個人/部署共有・解除を Phase 3.7 と同じ ReBAC 枠で。
  - kind を持たせ Task 6.7/6.10 が同じテーブル・同じ共有API・同じ監査経路に乗る共通枠とする。
- **受け入れ条件**:
  - [ ] アーティファクトを作成し新バージョンを追記でき、過去バージョンが不変で取得できる
  - [ ] アーティファクトを個人/部署に共有・解除でき、権限のないユーザーが参照できない
...
- **仕様**:
  ...
  - 部署単位の共有は Task 6.1 の ReBAC を流用。template 適用でチャット/ミニアプリの初期コンテキストを構成。
- **受け入れ条件**:
  - [ ] 5要素を持つ template を作成・更新（新バージョン）できる
  - [ ] template を部署に共有でき、過去バージョンを参照できる
...
- **依存**: 6.5, 6.9
- **仕様**:
  - **ミニアプリ = prompt template ＋ UIスペック ＋ 許可ツール＋宣言的バックエンド束縛**のバージョン付きアーティファクト
    （artifact kind=mini_app）。Task 6.1 の version＋ReBAC＋監査の共通枠にそのまま乗せる。
