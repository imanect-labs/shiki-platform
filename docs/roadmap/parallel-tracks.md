...
- **依存**: SK.6
- **仕様**:
  - 各サービスが**自前で計測した集約使用量**（shiki=LLMトークン/コスト＝llm-gateway が LiteLLM Proxy 計数を源泉に計上、skillex=DLC/LLM利用量）を、
...
- **仕様**:
  ...
  - llm-gateway のトークン会計（Phase 3・LiteLLM Proxy 計数が源泉）と監査を集計元に再利用する。
- **受け入れ条件**:
  - [ ] テナント別の使用量が正確に集計される
