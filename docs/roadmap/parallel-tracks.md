...
>
> **外部コミットメント**: 岩手県警察本部向け調達仕様書案（2026-08-24）6-3(7)・別紙1 L-1〜L-7 に
> 必須要件として提示済み。性能要件は「認識結果の表示まで 10 秒以内・同時利用者数と同数の同時入力」。
> **GPU は追加せず、対話用 GPU（RTX PRO 6000 96GB × 1／ノード）に CUDA MPS で同居させる前提**で
> 見積を出しており、実機計測で成立の見込みを確認済み。
