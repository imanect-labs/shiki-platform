...
- **目的**: **SaaS は優先ターゲット**（requirements §1.1）。共有コントロールプレーン＋**フルプールのデータプレーン**（design §4.1）と
  認可コンテキストの `tenant_id` は **Phase 0 で day-1 導入済み**。本トラックはその上に
  **テナント分離の強制・オンボーディング自動化・課金/メータリング・プラン制限**を載せる。
  データプレーン完全相乗り（フルプール）は **SAAS.5 で達成済み・既定トポロジ**。
- **タイミング**: 課金/オンボーディング/プラン制限は SaaS 提供に向けて随時。専用ストア（cell）への昇格と
  テナント単位プール分割は、専用環境契約 or 数千社規模になったら。
...
### Task SAAS.5: データプレーン完全相乗り（フルプール）— ✅ 達成済み・**既定トポロジ**
...
- 📌 **2026-07 正本化**: 本タスクの達成をもって**フルプールが SaaS の既定トポロジ**となった（cell は専用環境契約の
  オプションへ降格）。requirements/roadmap/phase-0/phase-8 側に残っていた「cell 隔離が正」の記述は実装と乖離して
  いたため一斉に修正済み。**この前提は「テナント数に比例して物理オブジェクト（索引・テーブル）が増える設計を
  禁じる」という制約を全フェーズへ課す**（→ Phase 13・[data-platform.md](../data-platform.md)）。
- **受け入れ条件**:
