...
## トラックSK — skillex 認証統合（接頭辞 SK.x）

- **目的**: shiki の Keycloak を**共有アイデンティティプール**として、職業訓練VRシステム skillex を
  フェデレートさせる。skillex の DLC/LLM 利用に必要なトークンを shiki 側 Keycloak が発行し、
  skillex ユーザーと shiki ユーザーを同一プールで運用する。**認可の真実は shiki が保持**する。
- **タイミング**: **Phase 0 の認証が安定したら並行**で進める。skillex は並行進行中のため、
  shiki 側の作業が skillex 開発を**ブロックしないこと**を最優先する。Phase 0 の認証設計に
  「共有プール・トークン発行」の継ぎ目を最初から織り込んでおく。
...
| SK.3 | 共有プール運用（skillex⇄shiki ユーザー同一プール、認可は shiki 保持） | auth | – | SK.1 |
...
### Task SK.3: 共有プール運用（同一プール・認可は shiki 保持）
- **area**: auth / **path**: `crates/auth`, `crates/authz`
- **依存**: SK.1
- **仕様**:
  - skillex ユーザーと shiki ユーザーを**同一アイデンティティプール**で運用しつつ、
    認可（ReBAC タプル＝認可の真実）は **shiki 側 authz store が一元保持**する。
  - skillex 由来ユーザーに shiki リソースへのアンビエント権限が漏れないよう、
    認可コンテキスト（`principal + org`）の評価で skillex 利用とshiki 利用を区別する。
- **受け入れ条件**:
  - [ ] skillex ユーザーが shiki リソースに既定で権限を持たない
  - [ ] 認可判定が shiki の authz store のみで決まる
  - [ ] 同一ユーザーが両システムで同一 principal として扱われる
