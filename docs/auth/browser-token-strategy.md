...
2. **セッション/プリンシパル単位の失効が即時に効かない**: JWT は `exp` まで有効なので、漏洩トークンの無効化・アカウント無効化・強制ログアウト・埋め込みクレームの陳腐化に即時対応できない（PIT-27 の剥奪 stale）。
   ⚠️ ただし**個別リソースの共有解除（phase-1 Task 1.6）はトークン/セッション形式に依らず OpenFGA のリクエスト毎チェック（＋PIT-11 `HIGHER_CONSISTENCY`）で担保される**ので、これは「JWT の弱点」ではない。オパーク化が解くのは前者（セッション/プリンシパル単位の失効）であって Task 1.6 ではない、という役割分担に注意。
3. **副次**: SSE で Cookie が自動添付されヘッダ注入が不要になる。ただし **POST で発話を送るチャットストリーム（phase-3 Task 3.5）は `EventSource` が GET 専用・body 不可のため、Cookie 化だけでは解決しない**（fetch-stream 維持か「POST で stream 作成 → GET `EventSource` で購読」への分離が必要）。
...
| セッション/プリンシパル失効の即時性（漏洩・退職・強制ログアウト）※リソース共有解除=Task 1.6 は別途 OpenFGA | 🟢 サーバセッション削除で即時 | 🔴 exp まで有効（弱点残存） | 🔴 exp まで有効 |
...
**推奨: (A)**。理由は (1) XSS 耐性、(2) **セッション/プリンシパル単位の即時失効**（漏洩トークン無効化・退職・強制ログアウト＝PIT-27 の剥奪 stale 解消）を満たすこと。※リソース共有解除（Task 1.6）は OpenFGA の責務で本件とは別レイヤ。ステートフル化はブラウザ⇄api 境界のみで、内部はJWTのままなので不変条件（単一チョークポイント・アンビエント権限禁止）は維持できる。
...
| refresh ロジック（`routes/auth/` or `middleware/session.rs` 内） | 新規 | refresh token のサーバ側保管・access token 期限前のローテーション更新・refresh 失効時のセッション破棄 |
