...
| 当初の根拠 | 再評価 |
|---|---|
| ステートレス検証（エアギャップ NFR-2） | エアギャップ要件は「**外部依存ゼロ**」であり「**状態ゼロ**」ではない。各セルがローカルなセッションストアを持てば成立。ブラウザが JWT を持つ必然性にはならない |
| 顧客ごと隔離セル / マルチテナント | セッションをセル間で同期する要件は無い（セルは独立）。各セルが自前のセッションストアを持てば済む |
...
| エアギャップ / 隔離セル | 🟢 各セルのローカルセッションストアで成立 | 🟢 影響なし | 🟢 影響なし |
| 内部/サービス間のステートレス性 | 🟢 維持（BFF がJWT転送／境界のみステートフル） | 🟢 維持 | 🟢 維持 |
| 追加インフラ | 🔴 セッションストア（Redis or Postgres） | 🟢 不要 | 🟢 不要 |
...
| `deploy/compose/docker-compose.yml` | 追加 | セッションストア（Redis、または Postgres セッションテーブル） |
...
1. **(A) フル BFF か、(中間) httpOnly Cookie 先行か**。失効即時性（Task 1.6）をいつ満たすか。
2. **セッションストアの実体**: Redis 追加 か Postgres セッションテーブル か（オンプレ縮退・PIT-29 と整合させる）。
3. **`SameSite` の強さ**（Strict/Lax）と CSRF トークン方式。
4. design.md §4.1 の更新文面（確定後）。
