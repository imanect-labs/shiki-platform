...
**依存**: Phase 11-pre（collab 基盤・下書きフロー）・Phase 1（ストレージ）・Phase 2（RAG 還流）・Phase 9（B1 砂箱）。詳細: [phase-11.md](./roadmap/phase-11.md)
- **スライド自前実装**（GrapesJS 砂箱＋Yjs 共同編集・AI も参加者・pptx ネイティブエクスポート必須・design §4.8.3）。
  チャット「パワポを作成して」→スライド下書き・「表を作成して」→CSV 下書き（下書き確定型）。
- **選択→AI 指示**（ノート/CSV/スライドの3エディタ共通 SelectionContext）。
- **Collabora Online 統合**（ソース自前ビルド・OfficeSuite トレイト・WOPI ホスト=StorageService クライアント・
  共同編集は内蔵委任・AI は非ロック時ファイルレベル編集／ロック中は提案バージョン）。
- スプレッドシート×shiki script はスコープ外に据え置き（将来イシュー）。
- **成果物**: スライドの生成→共同編集→pptx 出力と、Office 文書の共同編集＋AI 編集が動き、
  保存が StorageService→RAG に還流する。
...
- **外部コミットメント**: 岩手県警察本部向け調達仕様書案（2026-08-24）6-3(8)・別紙1 M-1〜M-10 に
  必須要件として提示済み。性能要件は「送信から表示まで 3 秒以内・同時接続 1,000 人」。
...
| **音声入力（トラックASR）** | Phase 3 の後ならいつでも。**岩手県警納入までに必須** | マイクの発話を文字にして入力欄へ。認識は `SpeechToText` トレイト裏でオンプレ完結。対話用 GPU に CUDA MPS で同居（GPU 追加なし）。[parallel-tracks.md](./roadmap/parallel-tracks.md) |
...
- **M8（Phase 13–14・ポストアルファ）**: HTML ページ／サイト公開と、職員間メッセージ。
  アルファ後の位置づけだが、**Phase 14 とトラックASR は岩手県警への納入コミットメントがある**
  （調達仕様書案に必須要件として提示済み）。
