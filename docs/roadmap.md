...
| **LLM コストルーティング（ルールベース・接頭辞 LR）** | **3.2 以降ならいつでも**（既定 shadow で先行計測） | 「タスクを満たす最安モデル」をルールで選ぶコスト最適化。前提工事（カタログのメタデータ化・複数プロバイダ）は Task 12.1 の素地にもなる。設計は [model-router.md](./llm/model-router.md)（design §4.5.1） |
