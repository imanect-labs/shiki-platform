...
本ドキュメント群には、確定設計に混じって **human の承認を待つ提案**が含まれる。実装着手前に確定させること。

- **OpenFGA relation スキーマの委譲モデリング**（engine.md §6）: 委譲を対象オブジェクトへの通常 relation タプル
  （subject=workflow プリンシパル）＋ `workflow_delegation` 行で管理する案。relation schema の追加は human 決定領域。
- **`crates/durable` の切り出しと名称**（engine.md §1）: chat の `generation_run`/`generation_event`（claim/リース/fencing/seq）
  実装を共有モジュールへ切り出す提案。クレート名は実装時に確定。
- **各種数値初期値の妥当性**: タイムアウト・プール数・上限・棚卸し周期などはすべて初期値（設定値）であり、
  実装時にベンチ・運用で調整する前提。妥当性の最終判断は human。
