...
- **青（チョークポイント）**: `StorageService` / `SearchService`(RAG) / `LlmGateway` /
  ...
  合流するだけである。
- **オレンジ（認可）**: OpenFGA。全チョークポイントが内部で同じ経路を通る。
- **ピンク（script-runtime）**: QuickJS on wasmtime。能力呼び出しは gRPC 双方向ストリームで
  `shiki-server` 側の `CapabilityNodeExecutor` に戻ってくる（script プロセス自体は資格情報ゼロ）。
...
| `effect_journal` | `0016_workflow_run.sql:109`（`0020` で `reserved_at` 追加） | **チョークポイント側**の冪等記録。内部能力ノード（storage/rag/llm/agent/workflow.start）の副作用を「高々 1 回」にする（PIT-31）。 | PK `(tenant_id, idempotency_key)`。`result_summary` が NULL の間は「予約済み・実行中」を表す。 |
...
| `schedule_occurrence` / `trigger_firing` / `scheduler_lease` | `0018_workflow_schedule.sql` | cron の 1 回の発火・イベントマッチの発火・リーダーリース。 | occurrence は `(workflow_id, scheduled_at)` UNIQUE で二重発火を防止。 |
...
「内部能力ノードに限り高々 1 回」まで引き上げるのが `effect_journal` テーブルと
`capability/journal.rs` の `JournalDecision`（`Proceed` / `AlreadyDone(結果)` /
`InProgress` / `DigestMismatch`）である。`idempotency_key`（`wf:{tenant_id}:{run_id}:{step_path}`。
`run/model.rs:103-106`）と `op_digest`（`sha256(api名 + 正規化パラメータ)`。`journal.rs:39-46`）の
組で `INSERT ... ON CONFLICT (tenant_id, idempotency_key)` を行い、同一キーでの再実行は
記録済み結果を no-op で返す。`result_summary` が NULL のままの行は「予約済み・副作用実行中」を
意味し、`RESERVATION_RECLAIM_SECS = 300`（`journal.rs:13`）を過ぎても NULL なら別ワーカーが
再取得してよいとみなす。外部 `http.request` はこの保護の対象外で **best-effort**
（`Idempotency-Key` ヘッダ注入支援のみ、README.md §6 Q4）。
...
2. **占有 TX・冪等発火**: `schedule_occurrence` の `(workflow_id, scheduled_at)` UNIQUE 制約で
   二重発火を防ぎつつ、`WorkflowRunLauncher` 経由で `workflow_run` を 1 行作る。
...
6. **AuthContext 分岐**: `ProdNodePorts::auth_ctx`（`workflow_runtime/ports.rs:43-60`）が
   `ec.principal_kind == "workflow"` なら `AuthContext::for_workflow`、そうでなければ
   `AuthContext::new(..PrincipalKind::User..)` を組み、チョークポイント（StorageService 等）に
   渡す。チョークポイント内部で通常どおり OpenFGA check が走る（ワークフロー専用の認可経路はない）。
