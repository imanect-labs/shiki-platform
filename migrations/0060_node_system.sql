-- 番号について: この migration は元は 0057 だったが、main 側の #393（共有リンクの個別取消）が
-- 同じ 0057 を先に取っていた（並行開発。どちらの PR の CI も相手を見ていなかった）。sqlx は
-- **バージョン番号**で照合するため、両方が揃うと VersionMismatch でサーバが起動しない。
-- 既存 DB に適用済みなのは main 側なので、こちらを 0060 へ繰り下げた。0058/0059 は
-- node.system に依存しないので、後ろへ回しても意味は変わらない。
