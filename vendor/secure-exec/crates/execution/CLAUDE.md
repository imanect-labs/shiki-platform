...
- Live guest stdin also has two delivery paths: `SECURE_EXEC_KEEP_STDIN_OPEN` uses `"stdin"` / `"stdin_end"` stream events, while TTY-style reads use `_kernelStdinRead` and must stay forwarded to the sidecar-backed kernel fd `0` pipe so timeout and EOF remain distinguishable.
...
- When adding a new raw V8 bridge method used by WASM host shims, keep `crates/execution/src/wasm.rs`, `crates/execution/src/v8_runtime.rs`, `crates/v8-runtime/src/session.rs`, `crates/bridge/bridge-contract.json`, and `packages/build-tools/bridge-src/` aligned, then rebuild `cargo build -p secure-exec-v8-runtime`; otherwise the method can compile cleanly while still being unavailable at runtime.
