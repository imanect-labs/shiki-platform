...
fn deep_research_plan_spec() -> serde_json::Value {
    serde_json::json!({
        ...
        "root": {
            ...
            "id": "dr-plan",
            "title": "調査計画",
            "intro": "この順に調べます。修正があれば書き足してください。",
            "submit": { "action": "start" },
            "submit_label": "この計画で開始",
            "allow_revise": true,
            "steps": [
                { "title": "視点を分けてクエリを作る", "description": "需要側・供給側・規制・批判的見解の 4 視点" },
                { "title": "一次ソースを取得して証拠台帳を作る", "description": "官公庁・IR を優先し、逐語引用を残す" },
                { "title": "社内文書を横断する", "description": "過去の調査資料と規程を確認する" },
                { "title": "節ごとに執筆して出典を検証する", "description": "証拠 ID に紐づかない主張を落とす" }
            ]
        }
    })
}
