...
impl RegistrationService {
    ...
    ///
    /// `declared_scopes` は**検査対象 run のピン済み IR の宣言スコープ**を渡す（現 registration の
    /// consented を渡すと declared ⊆ consented 検査が自明に真になり、再同意で縮小した scope の
    /// 検知が失われる）。
    pub async fn check_run_start(
        ...
        self.delegation
            .check_run_start(tenant_id, workflow_id, declared_scopes)
            .await
