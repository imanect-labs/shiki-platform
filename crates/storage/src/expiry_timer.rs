//! 共有リンク有効期限の失効タイマ（#342・イベント駆動・定期ポーリング無し）。
//!
//! OpenFGA タプルに TTL が無いため、期限切れの共有リンク（broad / redeem 済み per-user）を
//! バックグラウンドで剥奪する。**固定 interval ポーリングはしない**: 次に失効する時刻まで
//! sleep し、その瞬間だけ起きて処理する。新しい（今より早い）期限が設定されたら
//! [`StorageService::expiry_notify`] で起こされて次回起床時刻を再計算する。失効対象が無ければ
//! backstop 時間だけ idle する（安全網の再チェック）。

use std::sync::Arc;
use std::time::Duration;

use chrono::Utc;

use crate::service::StorageService;

/// 失効対象が無いときの最大 sleep（安全網。タイマ取りこぼしがあってもこの間隔で再チェックする）。
const BACKSTOP: Duration = Duration::from_hours(1);

/// 失効に失敗している（FGA/DB 障害等で期限切れ行が剥奪できず、次回起床時刻が過去のまま）ときの
/// 最小待機。0 で回すとログ洪水＋DB/FGA へのハンマリングになるため、必ずこの下限を挟む（B-1）。
const MIN_BACKOFF: Duration = Duration::from_secs(5);

/// 共有リンク失効タイマをバックグラウンド起動する（shiki-server の起動フローから呼ぶ）。
///
/// 返した `JoinHandle` は保持不要（プロセス生存中は動き続ける）。
pub fn spawn_share_link_expiry_timer(service: Arc<StorageService>) -> tokio::task::JoinHandle<()> {
    let notify = service.expiry_notify();
    tokio::spawn(async move {
        // 連続失敗時の指数バックオフ（健全なら MIN_BACKOFF にリセット・上限は BACKSTOP）。
        let mut backoff = MIN_BACKOFF;
        loop {
            // 1. 既に期限切れのものを剥奪する。失敗しても停止せず、待機側で backoff する。
            let revoke_failed = match service.revoke_expired_share_links(Utc::now()).await {
                Ok(n) if n > 0 => {
                    tracing::info!(count = n, "共有リンクの期限切れを剥奪しました（#342）");
                    false
                }
                Ok(_) => false,
                Err(e) => {
                    tracing::warn!(error = %e, "共有リンクの失効処理に失敗しました");
                    true
                }
            };
            // 2. 次に失効する時刻まで待つ。過去時刻が返る＝剥奪しきれていない（障害中）ので、全速
            //    ループを避けて backoff を挟む（B-1）。健全な将来期限/None なら backoff をリセットする。
            let wait = match service.next_share_link_expiry().await {
                Ok(Some(next)) => {
                    let ms = (next - Utc::now()).num_milliseconds();
                    if ms <= 0 || revoke_failed {
                        let w = backoff;
                        backoff = (backoff * 2).min(BACKSTOP);
                        w
                    } else {
                        backoff = MIN_BACKOFF;
                        Duration::from_millis(u64::try_from(ms).unwrap_or(u64::MAX))
                    }
                }
                Ok(None) => {
                    if revoke_failed {
                        let w = backoff;
                        backoff = (backoff * 2).min(BACKSTOP);
                        w
                    } else {
                        backoff = MIN_BACKOFF;
                        BACKSTOP
                    }
                }
                Err(e) => {
                    tracing::warn!(error = %e, "次回失効時刻の取得に失敗しました");
                    let w = backoff;
                    backoff = (backoff * 2).min(BACKSTOP);
                    w
                }
            }
            // 遠い将来の期限でも backstop 間隔で再チェックする（安全網）。
            .min(BACKSTOP);
            // 3. 起床時刻 or 新期限設定の通知のどちらか早い方まで待つ。
            tokio::select! {
                () = tokio::time::sleep(wait) => {}
                () = notify.notified() => {}
            }
        }
    })
}
