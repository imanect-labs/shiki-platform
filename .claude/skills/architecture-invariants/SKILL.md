...
- 全データアクセスは AuthContext { principal, org, tenant_id } を受け取っているか。アンビエント権限（暗黙のグローバル権限）に依存していないか。
- tenant_id を day-1 から保持しているか（SaaS マルチテナント前提）。tenant_id スコープの隔離境界を壊していないか。
