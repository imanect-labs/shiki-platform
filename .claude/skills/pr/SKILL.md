...
2. **全 bot（CodeRabbit・Codex）の指摘を必ず読む。** チェックの緑と「指摘なし」は別物 — bot はインラインコメントをスレッド解決済みにしないため、`gh pr checks` も未解消スレッド検出も**対応漏れを検出できない**。

   さらに **「最終コミット後のコメントが無い」＝「対応済み」ではない**。bot がまだ最新コミットを
   レビューしていないだけのことがある（push 直後は必ずこの状態）。`review-status.sh` は bot の
   `commit_id` と HEAD を突き合わせて未レビューを検出しブロックするが、手で確認する場合も
   **「見た結果、指摘が無い」のか「まだ見ていない」のか**を必ず区別すること。`review-status.sh` の「最終コミット後のコメント」欄が空でも、初回は生コメントを一読する:
