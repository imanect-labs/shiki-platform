...
劣化パターンは「自分がなぜそう書いたか」を知っていると正当化して読み飛ばす（「ここは件数が少ないから
N+1 でいい」など、実際には確かめていない前提で）。**意図を知らない目で掃き出させる。**

`subagent_type: "general-purpose"`・`model: "opus"`。**B・C の実測は委譲しない**（起動・計測は自分でやる方が速い）。
...
先に読むこと:
- <ROOT>/.claude/skills/pr/references/perf.md の「A. 静的観点」全体
- <ROOT>/CLAUDE.md の「コーディング規約」

観点: perf.md の A に列挙された劣化パターンが、この diff に含まれていないか。
インデックスの有無は <ROOT> の migrations を実際に読んで確認すること。
「件数が少ないから問題ない」と判断する場合は、上限を保証しているコード
（LIMIT・バリデーション・型・DB 制約）を示すこと。示せないなら指摘として出す。

⚠️ RAG の post-filter は N+1 に見えるが design.md §4.3 が必須と定める二段 authz の一段目である。
   これを「N+1 だから pre-filter に寄せろ」と指摘してはいけない（セキュリティ後退になる）。

出力は次の形式のリストのみ。前置き・要約は書かない。所見が無ければ「所見なし」とだけ返す。

- file: <リポジトリ相対パス>:<行>
  pattern: <perf.md A のどのパターンか>
  failure: <どの入力・規模で何が起きるか。具体的に>
  fix: <どう直すかの案を 1 文で>
  confidence: high | medium | low

制約:
- 実際に劣化する条件を書けないものは出さない。「〜かもしれない」は出さない。
- スタイルの好み・命名は出さない。
- diff に含まれない既存コードは、diff がそれを踏むことで悪化する場合のみ出す。
- ファイルは変更しない。最大 8 件。
```

所見は Phase 4 の敵対検証と同じ扱いにする（`confidence: low` は捨ててよいが、`high`/`medium` は
コードを読んで裏を取ってから着手する）。
...
TMP=$(mktemp -d)
LOGD=$(mktemp -d)          # ログも実行ごとに分ける（固定パスだと並行実行で上書きし合う）
cleanup() {
...
for side in base head; do
  (cd "$TMP/$side/web" && pnpm install --frozen-lockfile && pnpm build) > "$LOGD/$side-build.txt" 2>&1 \
    || { echo "$side の build に失敗: tail -30 $LOGD/$side-build.txt"; exit 1; }
done
diff <(grep -E '[○ƒλ●]' "$LOGD/base-build.txt") <(grep -E '[○ƒλ●]' "$LOGD/head-build.txt")
echo "ビルドログ: $LOGD"    # 失敗調査のため残す（不要なら rm -rf "$LOGD"）
```
